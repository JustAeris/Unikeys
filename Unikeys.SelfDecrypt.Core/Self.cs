...
namespace Unikeys.SelfDecrypt.Core;

public static class Self
{
    /// <summary>
    /// 4096 fixed pattern generated with <see cref="RandomNumberGenerator"/> to determine when to start decrypting
    /// </summary>
    ...
    public static string FileName { get; } = System.Diagnostics.Process.GetCurrentProcess().MainModule?.FileName
                                             ?? throw new InvalidOperationException("Could not get current process file name");

    private static IEnumerable<byte> GetMarkerBytes() => Convert.FromBase64String(Marker);

    /// <summary>
    /// Decrypts the current self-decryptable file
    /// </summary>
    /// <param name="password">Password used for encryption</param>
    /// <exception cref="InvalidOperationException">Marker could not be find, is this a self-decryptable file?</exception>
    private static void Decrypt(string password)
    {
        using var stream = new FileStream(FileName, FileMode.Open, FileAccess.Read);
        var markerIndex = FindPatternIndex(stream, GetMarkerBytes().ToArray());
        if (markerIndex == -1)
            throw new InvalidOperationException("Could not find marker in file, is this a self-decryptable file?");

        stream.Seek(markerIndex + 4096, SeekOrigin.Begin);

        EncryptionDecryption.DecryptFile(stream, FileName.Replace(".unikeys.exe", ""), password);
    }
    ...
    /// </summary>
    /// <param name="password">Password used for encryption</param>
    /// <returns>True if the operation succeeded, false if the current file doesn't have data to decrypt.</returns>
    public static bool TryDecrypt(string password)
    {
        try
        {
            Decrypt(password);
    ...
    /// <summary>
    /// Finds the index of a pattern in a stream
    /// </summary>
    /// <param name="stream">Stream to search in</param>
    /// <param name="pattern">Pattern to look for</param>
    /// <returns>Index of the pattern, -1 if can't be find</returns>
    private static long FindPatternIndex(Stream stream, IReadOnlyList<byte> pattern)
    {
        ...
        while (true)
        {
            var bytesReadNow = stream.Read(buffer, 0, bufferSize);
            ...
            for (var i = 0; i < bytesReadNow; i++)
            {
                if (buffer[i] == pattern[patternIndex])
                {
                    if (++patternIndex == pattern.Count)
                    {
                        return stream.Position - bytesReadNow + i - pattern.Count + 1;
                    }
                }
                else
                {
            ...
            if (bytesRead < pattern.Count) continue;
            bytesRead -= bufferSize;
            stream.Seek(bytesRead >= bufferSize ? -bufferSize : -bytesRead, SeekOrigin.Current);
        }
    }
}
